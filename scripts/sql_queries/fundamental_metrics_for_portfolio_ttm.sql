...
--		 	shares_fy s ON s.company_id = c.company_id AND s.date = c.date
		WHERE
			...
			i.symbol IN ('META','MSFT','UNH','AAPL','PM','V','GOOGL','TPL','PG','JNJ','CVX','ABBV','CP','EXP','KO','SSD','NVDA','MLI','AXP','AMZN','TMO','PYPL','ADBE','O','HON','COST','ADP')),
	ttm AS
...
SELECT
	...
	tt.median_adj_fcf_last_5_years,
...
FROM
	(SELECT
	    ...
	    PERCENTILE_DISC(0.5) WITHIN GROUP (ORDER BY interest_coverate_ratio) AS median_interest_coverage_ratio_last_5_years,
	 	...
	 	PERCENTILE_DISC(0.5) WITHIN GROUP (ORDER BY adj_fcf) AS median_adj_fcf_last_5_years
	FROM
		t
	GROUP BY
		t.company_id) AS tt
...
ORDER BY median_gross_margin_last_5_years DESC
