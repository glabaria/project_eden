...
def main():
    ...
    create_income_statement_table(connection, table_name="income_statement_fy",
                                  foreign_key_ref_tuple=("company_id", "company", "id"))
    create_balance_sheet_table(connection, table_name="balance_sheet_fy",
                                  foreign_key_ref_tuple=("company_id", "company", "id"))
    create_cash_flow_statement_table(connection, table_name="cash_flow_statement_fy",
                               foreign_key_ref_tuple=("company_id", "company", "id"))
    ...
    for root, dirs, files in os.walk(os.path.join(LOCAL_BASE_FILEPATH, "balance_sheets")):
        for file_name in files:
            if file_name.endswith('.csv'):
                try:
                    df = pd.read_csv(os.path.join(LOCAL_BASE_FILEPATH, "balance_sheets", file_name), usecols=["symbol"])
                except:
                    print(f"error with {file_name}")
                    continue
                insert_record(cursor, "company", ["symbol"], [df["symbol"].values[0]])
    load_csvs_and_insert(os.path.join(LOCAL_BASE_FILEPATH, "income_statements"), cursor, "income_statement_fy")
    load_csvs_and_insert(os.path.join(LOCAL_BASE_FILEPATH, "balance_sheets"), cursor, "balance_sheet_fy")
    load_csvs_and_insert(os.path.join(LOCAL_BASE_FILEPATH, "cash_flows"), cursor, "cash_flow_statement_fy")
    cursor.close()
